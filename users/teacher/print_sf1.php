...
$filter_section_id = $_GET['section_id'] ?? ''; // Kinuha ang section_id mula sa URL

$adviser_name = '';
$grade_level = '';
$section_name = '';

// KUNIN ANG DETALYE NG SECTION AT PANGALAN NG ADVISER
if (!empty($filter_section_id)) {
    $sec_query = "SELECT s.section_name, s.section_grade, t.teacher_name 
                  FROM section_tbl s 
                  LEFT JOIN teacher_tbl t ON s.section_id = t.section_id AND t.teacher_type = 'Class Adviser'
                  WHERE s.section_id = '$filter_section_id'";
    $sec_res = mysqli_query($conn, $sec_query);
    if ($sec_row = mysqli_fetch_assoc($sec_res)) {
        $grade_level = $sec_row['section_grade'];
        $section_name = $sec_row['section_name'];
        $adviser_name = strtoupper($sec_row['teacher_name']); // Gawing uppercase ang pangalan ni teacher
    }
}
?>
...
<body onload="window.print()">
    ...
    <table class="school-info">
        ...
        <tr>
            ...
            <td>Grade Level</td>
            <td><span class="info-data">Grade <?= htmlspecialchars($grade_level) ?> -
                    <?= htmlspecialchars($section_name) ?></span></td>
        </tr>
    </table>
    ...
    <table class="report-table">
        ...
        <tbody>
            <?php
            if (!empty($filter_school_year) && !empty($filter_section_id)) {
                // DINAGDAGAN: Kasama na ang section_id filter dito para tama ang lumabas na bata
                $sql = "SELECT * FROM student_tbl
                        ...
                        AND student_tbl.section_id = '$filter_section_id'
    ...
    <div class="footer-container">
        <table class="summary-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td><?= $maleCount ?? 0 ?></td>
                    <td><?= $femaleCount ?? 0 ?></td>
                    <td><?= ($maleCount ?? 0) + ($femaleCount ?? 0) ?></td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="signature-section">
            ...
            <div style="font-size: 14px; font-weight: bold; text-transform: uppercase;">
                <?= !empty($adviser_name) ? htmlspecialchars($adviser_name) : '____________________________________' ?>
            </div>
